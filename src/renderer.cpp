...
void renderer_draw(Renderer* renderer) {
    ...
    const uint32_t    frame_index            = renderer->curr_frame % 3;
    ...
    vkAcquireNextImageKHR(vk_ctx->device, swapchain_ctx->swapchain, UINT64_MAX, current_frame->image_available_semaphore, nullptr,
    ...
    const VkImageMemoryBarrier2 resolve_draw_image_memory_barrier = vk_lib::image_memory_barrier_2(
        swapchain_ctx->images[swapchain_image_index], subresource_range, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
    ...
    const VkImageMemoryBarrier2 resolve_present_image_memory_barrier = vk_lib::image_memory_barrier_2(
        swapchain_ctx->images[swapchain_image_index], subresource_range, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
        vk_ctx->queue_family, vk_ctx->queue_family, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR,
    ...
    VK_CHECK(vkQueueSubmit2(vk_ctx->graphics_queue, 1, &current_frame->submit_info_2, current_frame->in_flight_fence));
    ...
    vkQueuePresentKHR(vk_ctx->present_queue, &present);
